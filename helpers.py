...
def call_app_with_payment(client, private_key, index, app_args, amount):
    ...
    payTxn = transaction.PaymentTxn(sender, params, appAddr, amount)

    # create unsigned transaction
    txn = transaction.ApplicationNoOpTxn(sender, params, index, app_args)

    transaction.assign_group_id([payTxn, txn])

    signedPayTxn = payTxn.sign(private_key)
    signedAppCallTxn = txn.sign(private_key)

    client.send_transactions([signedPayTxn, signedAppCallTxn])

    transaction.wait_for_confirmation(client, txn.get_txid())
...
def close_out_app(client, private_key, index):
    ...
    print("Closed out from app-id: ",
          transaction_response["txn"]["txn"]["apid"])
